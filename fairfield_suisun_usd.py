...
import core
from common.misc import todays_date
from common.text_utils import get_hash
from core.bid_scraper_plus import BidScraper, SkipBid
from core.common import datefmt, download_google_folder_files
...
class Main(BidScraper):
    ...
    def pre_execute(self):
        ...
        a = self.soup.select('.column > .block ul p a[target*="_blank"]')
        self.curr_bid = [
    ...
    def fetch_rows(self):
        if self.status == "closed":
            return
        yield from self.curr_bid + lbl if (
            lbl := re.findall(
    ...
    def scrape_bid(self, bid, row):
        def get_due_date_from_file(soup):
            ...
            if not dd and self.status['open']:
        ...
        if detail_res := self.get(bid.sourceURL):
            ...
            if not dueDate and self.status == "closed":
                bid.dueDate = bid.set_default_due_date()
        else:
